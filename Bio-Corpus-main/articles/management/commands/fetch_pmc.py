...
from fetch_scripts import pmc
from articles.services import upsert_article


class Command(BaseCommand):
    help = "Fetch full-text Open Access articles from PubMed Central per domain."
    ...
    def handle(self, *args, **opts):
        ...
        for key, cfg in domains.items():
            self.stdout.write(self.style.NOTICE(
                f"\n[PMC] Domaine '{key}' — {per} articles..."))
            try:
                ids = pmc.search_pmcids(cfg["query"], retmax=per)
                self.stdout.write(f"  → {len(ids)} PMC IDs. Téléchargement...")
                articles = pmc.fetch_articles(ids)
        ...
        self.stdout.write(self.style.SUCCESS(
            f"\n=== {total_new} nouveaux articles PMC ==="))
